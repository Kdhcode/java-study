...
public class StudentListExample {
	...
	public static void main(String[] args) {
		...
		studentList.add(new Student("김씨", 21));
		studentList.add(new Student("박씨", 13));
		studentList.add(new Student("이씨", 32));
		
		// 객체 검색
		// 사용자 정의 객체를 사용할 경우 indexOf(), contains() 가 
		// 해당 객체 존재 여부를 판단하는 기준은 equals()
		// equals() 메소드를 재정의(오버라이딩) 해야 동등 객체로 판단
		if (studentList.indexOf(new Student("김씨", 21)) != -1) {
			System.out.println("indexOf: 있다.");
		} else {
			System.out.println("indexOf: 없다.");
		}
		
		if (studentList.contains(new Student("김씨", 21))) {
			System.out.println("indexOf: 있다.");
		} else {
			System.out.println("indexOf: 없다.");
		}
