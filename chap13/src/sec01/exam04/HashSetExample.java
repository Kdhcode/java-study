...
//		Set<String> set = new HashSet<String>();
		...
		while (iterator.hasNext()) { // 가져올 객체가 있으면 true, 없으면 false
			...
			if ("Java".equals(element)) {
				iterator.remove();
				// 참고: remove()는 next() 호출 이후에만 호출 가능하면, 한 번만 가능
			}
		...
		// 두 집합(HashSet)의 합지합, 교집합, 차집합 연산 가능
		...
		// 이때 2개의 HashSet에 공통적으로 포합된 데이터는 제거
...
//		System.out.println("교집합: " + class1);
		...
		// 차집합
		// 기준 HashSet에서 대성 HashSet과의 교집합 제외
