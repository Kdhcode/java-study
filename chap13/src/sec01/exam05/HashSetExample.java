...
//		set.remove(new Member("김재현", 30));
		...
		// 정렬 기준으로 어떤 값을 사용 할지 별도로 구현해야 함
		...
		// 1. Set -> List 변환
		List<Member> memberms = new ArrayList<>(members);
		
		// 2. 정렬(나이 오름 차순)
		Collections.sort(memberms);
		
		// 3. 출력(향상된 for문)
		for (Member m : memberms) {
			System.out.println(m);
		}
		
		//다시 Set으로 변환(정렬된 순서 유지하려면 LinkedHashSet)
		Set<Member> members2 = new LinkedHashSet<Member>(memberms);
		...
		// Quiz
		Set<Member> members3 = new LinkedHashSet<Member>(memList);
		memList = new ArrayList<Member>(members3);
