...
public class CalendarExample {
	...
	public static void main(String[] args) {
		...
		case Calendar.FEBRUARY: strWeek = "금"; break;
...
//		case Calendar.SUNDAY: strWeek = "일"; break;
		...
		// 참고 : 자바 8 이후 권장 방식
		// Calender 대신 java.time API로 대체 가능 
		// 현재 날짜/시간 얻기
		...
		int day2 = now2.getDayOfMonth(); // 월: 1~31
		...
		String strAmPm2 =  hour24 < 11 ? "오전" :"오후";
		System.out.println(strAmPm + " ");
		
		// 12시간제 시각: 1~12
		int hour12 = hour24 % 12;
		if(hour12 == 0) hour12 = 12; // 24시간제 기준으로 0시, 12시 일때 -> 12시
		...
		// 날짜/시간 비교 : isBefore(), inAfter(), isEqual()
		// 날짜 비교
		LocalDate today = LocalDate.now();
		LocalDate tagetDate = LocalDate.of(2025, 12, 25); // 2025년 12월 25일
		if (today.isBefore(tagetDate)) {
			System.out.println("오늘은 크리스마스 전입니다.");
		} else if(today.isAfter(tagetDate)) {
		...
		// 시간 비교
		LocalDateTime todaTime = LocalDateTime.now();
		LocalDateTime deadline = LocalDateTime.of(2024, 8, 5, 18, 0); // 2025년 8월 4일 오후 6시
		
		if(todaTime.isBefore(deadline)) {
			System.out.println("마감 시간 전입니다. 작업을 계속하세요.");
		} 
		else if(todaTime.isAfter(deadline)) {
			System.out.println("마감 시간 이후입니다. 제출을 마감합니다.");
		} else {
